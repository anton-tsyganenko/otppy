...
`-z action`, `--gzip=action` - `compress`(`c`) or `decompress`(`d`) data using gzip, `no` - don't use or `auto` - don't compress data, but decompress if it looks compressed, by default.  
